...
class AlphaSigmoid(tf.keras.layers.Layer):
   def __init__(self, nr_gaussians, param_size):
      super(AlphaSigmoid, self).__init__()   
      self.nr_gaussians = nr_gaussians
      self.param_size = param_size
      
      self.mat_alpha, self.mat_mu_sigma = self.create_select_matrix()

   def call(self, x):
      alpha = tf.linalg.matmul(x, self.mat_alpha)
      
      alpha_first = alpha[:, 0]
      alpha_rest = tf.math.sigmoid(alpha[:, 1:])
      alpha = tf.concat([alpha_first[:, tf.newaxis], alpha_rest], axis=1)
      
      mu_sigma = tf.linalg.matmul(x, self.mat_mu_sigma)
      return tf.concat([alpha, mu_sigma], axis=1)   
   
   def create_select_matrix(self):      
      mat_alpha = tf.eye(self.nr_gaussians)
      mat_alpha = tf.pad(mat_alpha, [[0, self.param_size - self.nr_gaussians], [0, 0]])

      mat_mu_sigma = tf.eye(self.param_size-self.nr_gaussians)
      mat_mu_sigma = tf.pad(mat_mu_sigma, [[self.nr_gaussians, 0], [0, 0]])
      
      mat_alpha = tf.cast(mat_alpha, tf.float64)
      mat_mu_sigma = tf.cast(mat_mu_sigma, tf.float64)
      
      return mat_alpha, mat_mu_sigma


class Disambiguate(tf.keras.layers.Layer):
   def __init__(self, nr_gaussians, param_size):
      super(Disambiguate, self).__init__()     
      self.nr_gaussians = nr_gaussians
      self.param_size = param_size
   ...
   def call(self, x):
      alpha  = tf.linalg.matmul(x, self.mat_alpha)
      alpha = tf.math.cumprod(alpha, axis=1)
...
class MDN(tf.keras.Model):
   def __init__(self, nr_gaussians, nr_neurons, activation_function, symmetrize=False, include_b=False, disambiguate=False):
      ...
      if self.disambiguate: 
         self.alpha_activation = AlphaSigmoid(self.nr_gaussians, self.params_size)
         self.disambiguate = Disambiguate(self.nr_gaussians, self.params_size)
...
def build_model(nr_gaussians=20, activation_function='relu', nr_neurons=8, learning_rate=1e-4, symmetrize=False, include_b=False, disambiguate=False): 
    negloglik = lambda y, p_y: -p_y.log_prob(y)

    model = MDN(nr_gaussians, nr_neurons, activation_function, symmetrize=symmetrize, include_b=include_b, disambiguate=disambiguate)
...
def load_model(model_path, config_path=None):
   ...
   model = build_model(config.get("nr_gaussians"), 
                       ...
                       disambiguate=config.get("disambiguate"))
